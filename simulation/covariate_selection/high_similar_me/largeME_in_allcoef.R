...
mos=0.2*10 #here it is: max(me)/sdX
...
plot(delta_grid,MonteCarlo_NCmean_gmus,
     main="Elbow plot, ME setting 2",
     xlab = expression(delta),
     ylab = "No. nonzero coefficients",
     type="l",lty=1)
points(c(0.12,0.1,0.08,0.06,0.04),y=MonteCarlo_NCmean_gmus[c(7,6,5,4,3)],
       col="red",pch=20)
lines(delta_grid,MonteCarlo_NCmean_gmus0,
      lty=2)
points(c(0.12,0.1,0.08,0.06,0.04),y=MonteCarlo_NCmean_gmus0[c(7,6,5,4,3)],
       col="red",pch=20)
lines(delta_grid,MonteCarlo_NCmean_gmus00,
      lty=3)
points(c(0.12,0.1,0.08,0.06,0.04),y=MonteCarlo_NCmean_gmus00[c(7,6,5,4,3)],
       col="red",pch=20)

legend(0.3, 6, legend=c("CGMUS", "Equal ME CGMUS","GMUS"),
       lty=c(1,2,3), cex=0.7)
########################## plot done


delta_esti_gmus_006<-0.12 #NC ~= 2.03
delta_esti_gmus_005<-0.10 #NC ~= 2.28  
delta_esti_gmus_004<-0.08 #NC ~= 2.59 
delta_esti_gmus_003<-0.06 #NC ~= 3.01 
delta_esti_gmus_002<-0.04 #NC ~= 3.73

delta_esti_gmus0_008<- 0.12 #NC ~= 2.4
delta_esti_gmus0_006<- 0.10 #NC ~= 2.63   
delta_esti_gmus0_004<- 0.08 #NC ~= 2.97 
delta_esti_gmus0_003<- 0.06 #NC ~= 3.28
delta_esti_gmus0_002<- 0.04 #NC ~= 3.79

delta_esti_gmus00_014<-  0.12 #NC ~= 3.14
delta_esti_gmus00_010 <- 0.10 #NC ~= 3.38
delta_esti_gmus00_008<-  0.08 #NC ~= 3.69
delta_esti_gmus00_006<-  0.06 #NC ~= 4.05 
delta_esti_gmus00_004<-  0.04 #NC ~= 4.54   
...
for (bstar in 1:B){
  ...
  lasso_beta<-rbind(lasso_beta,t(as.vector(logistic_lasso_fit$beta)))
...
#summary_estbeta(gmus_beta_006)[summary_estbeta(gmus_beta_006)[,3]!=0,]
