...
for(MOS in 1:length(mos)){
  ...
  set.seed(1)
  me<-mos/10*c(abs(rnorm(s,mean=1,sd=0.2)),10*abs(rnorm(p-s,mean=1,sd = 0.2)) )
...
plot(delta_grid,MonteCarlo_NCmean_gmus,
     main="Elbow plot, ME setting 4",
     xlab = expression(delta),
     ylab = "No. nonzero coefficients",
     type="l",lty=1)
points(c(0.14,0.12,0.10,0.08,0.06),y=MonteCarlo_NCmean_gmus[c(8,7,6,5,4)],
       col="red",pch=20)
lines(delta_grid,MonteCarlo_NCmean_gmus0,
      lty=2)
points(c(0.14,0.12,0.10,0.08,0.06),y=MonteCarlo_NCmean_gmus0[c(8,7,6,5,4)],
       col="red",pch=20)
lines(delta_grid,MonteCarlo_NCmean_gmus00,
      lty=3)
points(c(0.14,0.12,0.10,0.08,0.06),y=MonteCarlo_NCmean_gmus00[c(8,7,6,5,4)],
       col="red",pch=20)

legend(0.3, 35, legend=c("CGMUS", "Equal ME CGMUS","GMUS"),
       lty=c(1,2,3), cex=0.7)
...
delta_esti_gmus_006<-0.14 #NC ~= 7.5 
delta_esti_gmus_005<-0.12 #NC ~= 9.1  #elbow
delta_esti_gmus_004<-0.10 #NC ~= 11.3 #elbow
delta_esti_gmus_003<-0.08 #NC ~= 15.5 
delta_esti_gmus_002<-0.06 #NC ~= 21.9

delta_esti_gmus0_008<- 0.14 #NC ~= 9
delta_esti_gmus0_006<- 0.12 #NC ~= 11   #elbow
delta_esti_gmus0_004<- 0.10 #NC ~= 13.7 #elbow
delta_esti_gmus0_003<- 0.08 #NC ~= 17.7
delta_esti_gmus0_002<- 0.06 #NC ~= 24.0

delta_esti_gmus00_014<-  0.14 #NC ~= 10.6
delta_esti_gmus00_010 <- 0.12 #NC ~= 11.4
delta_esti_gmus00_008<-  0.10 #NC ~= 12.7
delta_esti_gmus00_006<-  0.08 #NC ~= 14.4 #elobw
delta_esti_gmus00_004<-  0.06 #NC ~= 17   #elbow
...
#colMeans(summary_estbeta(gmus_beta_006)) #0.14 #NC ~= 7.5
#colMeans(summary_estbeta(gmus_beta_005)) #0.12 #NC ~= 9.1 elbow
#colMeans(summary_estbeta(gmus_beta_004)) #0.10 #NC ~= 11.3 elbow
#colMeans(summary_estbeta(gmus_beta_003)) #0.08 #NC ~= 15.5
#colMeans(summary_estbeta(gmus_beta_002)) #0.06 #NC ~= 21.9

#colMeans(summary_estbeta(gmus0_beta_008)) #0.14 #NC ~= 9
#colMeans(summary_estbeta(gmus0_beta_006)) #0.12 #NC ~= 11 elbow
#colMeans(summary_estbeta(gmus0_beta_004)) #0.10 #NC ~= 13.7 elbow
#colMeans(summary_estbeta(gmus0_beta_003)) #0.08 #NC ~= 17.7
#colMeans(summary_estbeta(gmus0_beta_002)) #0.06 #NC ~= 24

#colMeans(summary_estbeta(gmus00_beta_014)) #0.14 #NC ~= 10.6
#colMeans(summary_estbeta(gmus00_beta_010)) #0.12 #NC ~= 11.4
#colMeans(summary_estbeta(gmus00_beta_008)) #0.10 #NC ~= 12.7
#colMeans(summary_estbeta(gmus00_beta_006)) #0.08 #NC ~= 14.4 elbow
#colMeans(summary_estbeta(gmus00_beta_004)) #0.06 #NC ~= 17 elbow
