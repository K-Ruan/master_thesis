...
cvloglik_gmus_result<-cvloglik_gmus_result$x #1cvloglik(5-fold-cv)60samples * B=50 times)* 10 mos
cvloglik_gmus0_result<-cvloglik_gmus0_result$x #1cvloglik(5-fold-cv)60samples * B=50 times)* 10 mos
cvloglik_gmus00_result<-cvloglik_gmus00_result$x #1cvloglik(5-fold-cv)60samples * B=50 times)* 10 mos
cvloglik_gds_result<-cvloglik_gds_result$x #1cvloglik(5-fold-cv)60samples *  B=50 times)* 10 mos

#60samples * B=50 *10 mos

split1<-seq(1,length(cvloglik_gmus_result),by=60) #1:30000
split2<-seq(60,length(cvloglik_gmus_result),by=60)# 1:30000
...
for(i in 1:length(split1)){
  mean_cvloglik_gmus<-c(mean_cvloglik_gmus,mean(cvloglik_gmus_result[split1[i]:split2[i]]))
}

for(i in 1:length(split1)){
  mean_cvloglik_gmus0<-c(mean_cvloglik_gmus0,mean(cvloglik_gmus0_result[split1[i]:split2[i]]))
}

for(i in 1:length(split1)){
  mean_cvloglik_gmus00<-c(mean_cvloglik_gmus00,mean(cvloglik_gmus00_result[split1[i]:split2[i]]))
}

for(i in 1:length(split1)){
  mean_cvloglik_gds<-c(mean_cvloglik_gds,mean(cvloglik_gds_result[split1[i]:split2[i]]))
}
...
ggplot(df_combined,aes(x=mos,y=mean_loglik, fill=type)) +
  geom_boxplot() +
  labs(title = "ME Setting 2, n=60", x="Constant C")
