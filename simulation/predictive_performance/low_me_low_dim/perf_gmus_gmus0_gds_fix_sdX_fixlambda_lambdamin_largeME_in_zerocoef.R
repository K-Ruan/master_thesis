...
cvloglik_gmus<-c()  ##   1cvloglik(5-fold-cv) 200 * B times)* 10 mos
cvloglik_gmus0<-c() ##   1cvloglik(5-fold-cv) 200 * B times)* 10 mos
cvloglik_gmus00<-c() ##   1cvloglik(5-fold-cv) 200 * B times)* 10 mos
cvloglik_gds<-c() ##   1cvloglik(5-fold-cv)200  * B times)* 10 mos
...
for(MOS in 1:length(mos)){
  ...
  me<-mos[counts_nr]*c(rep(c(0.1,0.2,0.3),times=4)[1:10],rep(1,p-s))  
